...
function handleRequest(req, res) {
  try {
    ...
    if(reqUrl.pathname === "/download") {
      const signature = crypto.createHash("sha256").update(`${process.env.TOKEN}:${reqUrl.searchParams.get("filename")}:${reqUrl.searchParams.get("url")}`).digest().toString("hex");
      ...
      request(reqUrl.searchParams.get("url"), function(err, response, body) {
        ...
        res.writeHead(200, {
          ...
          "content-disposition": `attachment; filename=${reqUrl.searchParams.get("filename") || "file.zip"}`
        });
...
exports.listen = function() {
  this.server.listen(process.env.PORT || 8080, process.env.BIND || "0.0.0.0");
}
